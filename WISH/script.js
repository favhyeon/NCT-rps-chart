...
const CUSTOM_COLOR_KEY = "nctwish-custom-colors";
...
saveBtn.addEventListener("click", async () => {
    ...
    /* 모바일에서는 .capture-area가 반응형(좁은 폭) CSS로 화면에 떠
       ...
       그래서 캡처 직전에 실제 DOM의 인라인 width를 1100px로 강제로
       넓혀서, 어떤 기기에서 저장하든 항상 동일한 폭/비율로 리플로우된
       상태를 캡처하도록 한다. */
    ...
    try {
        const canvas = await html2canvas(area, {
            ...
            useCORS: true,
            logging: false,
            width: Math.max(DESKTOP_CAPTURE_WIDTH, area.scrollWidth),
            height: area.scrollHeight,
            windowWidth: Math.max(DESKTOP_CAPTURE_WIDTH, area.scrollWidth),
        ...
        const fileLabel = currentTab === "rps" ? "윗페스_취향표" : "공수_취향표";

        const link = document.createElement("a");
        link.href = currentBlobUrl;
        link.download = `NCTWISH_${fileLabel}.png`;
